...
# 设置停用词
stop_words = ['的', '了', '你', '我', '是', '吗', '都', '吧', '说', '就', '啊', '他们', '还', '可以']

# 词云展示
def visual_ciyun():
    pic = '../img.jpg'
    gen_stylecloud(text=result,
                   icon_name='fab fa-medium-m',
                   font_path='msyh.ttc',
...
def cipin_bar() -> Bar:
    c = (
        ...
        .add_xaxis(x1_data)
        .add_yaxis("知乎词频展示图", y1_data)
        .set_global_opts(
            title_opts=opts.TitleOpts(title="知乎词频展示图"),
...
def cipin_line() -> Line:
    c = (
        ...
        .set_global_opts(title_opts=opts.TitleOpts(title="知乎词频展示图"))
    )
    return c
...
def title():
    c = (
        ...
        .set_global_opts(
            title_opts=opts.TitleOpts(title="----------知乎无鸡可炸高频词汇排行榜----------",
...
def cipin_Funnel():
    ...
    c = (
        Funnel(init_opts=opts.InitOpts(width="1600px", height="800px", chart_id=6, bg_color='#ADD8E6'))
        .add(
            series_name="",
            data_pair=data,
            gap=2,
...
def visual_cipin():
    ...
    page.render("知乎词频展示图.html")
    page.save_resize_html('知乎词频展示图.html', cfg_file='chart_config.json', dest='知乎词频统计展示图666.html')
